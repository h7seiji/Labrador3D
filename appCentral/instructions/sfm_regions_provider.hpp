...
public:
    ...
    C_Progress *  my_progress_bar = nullptr)
  {
    ...
    #pragma omp single nowait
#endif
      {
        ...
        const std::string basename = iter->second->s_Img_path;
        ...
        {
          std::cerr << "Invalid regions files for the view: " << basename << std::endl;
          bContinue = false;
        }
        //else
#ifdef OPENMVG_USE_OPENMP
  ...
  /// CREATED FUNCTION
  /// store Region object and set region_type_ if not defined
  virtual bool guardar( IndexT iter, std::unique_ptr<features::Regions> region )
  {
	  if (!region_type_)
	  {
		  //std::cout << "Type not defined" << std::endl;
		  std::unique_ptr<features::Regions>& region_type = region;
		  region_type_.reset(region_type->EmptyClone());
	  }
	  cache_[iter] = std::move(region);
	  return true;
  }
